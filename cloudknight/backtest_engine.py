...
from .config import (
    ...
    BACKTEST_LOT_SIZE, BACKTEST_T_PLUS_ONE, BACKTEST_MIN_COMMISSION,
)
...
class BacktestEngine:
    ...
    def run_backtest(self, strategy_cls: Type[CloudKnightStrategy],
                     start_date: str = None, end_date: str = None,
                     stock_pool: List[str] = None,
                     verbose: bool = False) -> Optional[BacktestResult]:
        """使用 AKQuant 执行单策略回测
        ...
        if verbose:
            symbols = list(data.keys()) if isinstance(data, dict) else data["symbol"].nunique() if isinstance(data, pd.DataFrame) else 0
            logger.info(f"回测: {start_date} ~ {end_date}, {len(stock_pool) if stock_pool else symbols} 只股票")

        try:
            result = self._run_akquant_backtest(
                data=data, strategy_cls=strategy_cls,
                start_date=start_date, end_date=end_date,
            )
            if result is None:
                return None
            return self._parse_akquant_result(
                result, strategy_cls.name, start_date, end_date
            )
        except Exception as e:
            logger.error(f"AKQuant 回测失败: {e}")
            return None

    def compare_strategies(self, strategy_classes: List[Type[CloudKnightStrategy]],
                           start_date: str = None, end_date: str = None,
                           stock_pool: List[str] = None,
    ...
    def _prepare_data(self, stock_pool: List[str],
                      start_date: str, end_date: str) -> Optional[Dict[str, pd.DataFrame]]:
    ...
    def _run_akquant_backtest(self, data: Dict[str, pd.DataFrame],
    ...
    def _parse_akquant_result(self, raw_result, strategy_name: str,
        ...
        try:
            ...
            if metrics is not None and not metrics.empty:
                ...
                final_cap = self._safe_metric(metrics, cols_lower, ["final_value", "final_capital",
                                             "ending_value", "total_equity"])
                if final_cap == 0:
                    final_cap = initial_cap
                total_return = (final_cap / initial_cap - 1) * 100
                max_dd = self._safe_metric(metrics, cols_lower, ["max_drawdown",
                                           "max_drawdown_pct"]) * 100 if metrics is not None else 0
            ...
            if equity_df is not None and not equity_df.empty:
                daily_returns = equity_df["equity"].pct_change().dropna().tolist() if "equity" in equity_df.columns else []
