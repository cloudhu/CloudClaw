...
def _limit():
    ...
    if e < 0.8:
        time.sleep(0.8 - e + random.uniform(0, 0.3))
    _last_req = time.time()
...
def _http_list(stype):
    ...
    url = f"http://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=200&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs={fs}&fields=f2,f3,f4,f12,f14,f20"
...
def _http_stocks(bcode):
    _limit()
    url = f"http://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=200&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs=b:{bcode}%2Bf:!50&fields=f2,f3,f4,f5,f6,f8,f9,f12,f14,f20"
    r = _session().get(url, timeout=15)
    items = r.json().get("data", {}).get("diff", [])
    if not items:
        return pd.DataFrame()
...
def fetch_list(cache_mgr, force=False):
    ...
    for label, fetcher in sources:
        if label in _unavailable:
            continue
        try:
            df = fetcher()
            if df is not None and not df.empty:
                if label != sources[0][0]:
                    logger.info("  -> sector list via %s", label)
...
def fetch_stocks(cache_mgr, industry, board_code=""):
    ...
    for label, fetcher in sources:
        ...
        try:
            ...
            if df is not None and not df.empty:
                if label != sources[0][0]:
                    logger.info("  -> %s stocks via %s", industry, label)
