...
from .config import AUCTION_VOLUME_RATIO_MIN, AUCTION_PRICE_CHANGE_ALERT
...
class AuctionAnalyzer:
    ...
    def analyze_auction(self, watchlist: List[str],
                        verbose: bool = False) -> AuctionSnapshot:
        """对关注列表进行竞价分析
        ...
        for code in watchlist:
            try:
                ...
                if result:
                    if verbose:
                        logger.info(f"  {code} {result.name}: "
                                    f"{result.auction_pct:+.2f}% "
                                    f"[{result.strength}] {result.signal}")
    ...
    def _analyze_one(self, code: str) -> Optional[AuctionStockResult]:
        """分析单只股票的竞价表现
        ...
        try:
            df = self.fetcher.fetch_daily_kline(
                code, start_date=(datetime.now().replace(day=1).strftime("%Y%m%d"))
            )
            ...
            date_col = next((c for c in cols if "日期" in c or "date" in c.lower()), None)
    ...
    def filter_strong_auction(self, snapshot: AuctionSnapshot,
                              min_pct: float = 2.0) -> List[AuctionStockResult]:
        """筛出竞价强势的个股"""
        return [r for r in snapshot.results
                if r.strength == "strong" and r.auction_pct >= min_pct]

    def make_open_trade_decision(self, snapshot: AuctionSnapshot,
                                 watch_codes: Dict[str, List[str]]
